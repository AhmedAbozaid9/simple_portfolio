...
const ToggleTheme = ({theme,setTheme}) => {
  ...
  const icon = toggle ?  <BsSunFill size={16} color={"#FFC93C"}/> : <BsFillMoonFill size={16} color={"#3D1766"}/>
  ...
  return (
    ...
    >
      ...
      <div
        className={
          `bg-white sm:w-6 sm:h-6 h-5 w-5 rounded-full shadow-md transform duration-300 ease-in-out ${toggle ? null : toggleClass} flex items-center justify-center`
        }
      >{icon}</div>
    </div>
  );
